...
export default function Navbar({onReservation,onAbout,onHome,onServices,onChefs,onFoodMenu,onFooter,onReviews}){
    ...
    return(
        <div className={`text-white fixed w-full top-0 flex items-center z-50 ${moved ? 'bg-gray-800' :''}`}>
            <div className="flex items-center md:pl-10 py-3 pl-3">
                <span className="text-white ml-2 font-bold text-3xl">RK ServeHub</span>
            </div>
            <div className="flex-grow"></div>
            
            <button className="text-xl lg:hidden border rounded mr-4 p-1 overflow-hidden" onClick={toggleMenu}>{menuOpen ? <FaTimes/> : <FaBars/>}</button>
            ...
            <nav className={`absolute mr-8 lg:relative lg:flex wrap lg:items-center lg:w-auto w-full top-12 lg:top-0 lg:right-0 left-0 lg:bg-transparent lg:block bg-gray-800 md:justify-end transition-all duration-500 mt-2 ease-in-out ${menuOpen ? 'menu-open' : 'hidden'}`}>
                <ul className="flex flex-col lg:flex-row items-start lg:items-center gap-y-6 lg:gap-x-5 p-4 mt-2 md:ml-0 lg:p-0">
                    <p onClick={()=>{onHome(); closeMenu();}}>Home</p>
                    <p onClick={()=>{onAbout(); closeMenu()}}>About</p>
                    <p onClick={()=>{onServices(); closeMenu();}}>Services</p>
                    <p onClick={()=>{onFoodMenu(); closeMenu();}}>Menu</p>
                    <div className="relative">
                        <button onClick={toggleDropdown} className="text-gray-100 hover:text-[DodgerBlue] transition duration-500 flex items-center">
                            PAGES
                        ...
                        {isOpen && (
                            <div className="absolute left-0 mt-2 w-48 bg-gray-800 rounded-md shadow-lg z-10">
                                <p onClick={()=>{onChefs(); closeMenu();}} className="text-white block px-4 py-2 text-sm hover:bg-[DodgerBlue]">
                                    OurTeam
                                </p>
                                <p onClick={()=>{onReviews(); closeMenu();}} className="text-white block px-4 py-2 text-sm hover:bg-[DodgerBlue]">
                                    Reviews
                                </p>
                                <p onClick={()=>{onFooter(); closeMenu();}} className="text-white block px-4 py-2 text-sm hover:bg-[DodgerBlue]">
                                    Contact
                                </p>
                            </div>
                        )}
